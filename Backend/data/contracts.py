...
class DataRequest(BaseModel):
    ...
    requested_fields: List[RequestedField] = Field(
        default=[RequestedField.OHLC], description="What data to include"
    )
...
class DataMetadata(BaseModel):
    ...
    source: str = Field(
        ..., description="Data source name (e.g., 'yahoo', 'coingecko')"
    )
    fetched_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="When fetched"
    )
    is_complete: bool = Field(default=True, description="All requested data present?")
    missing_dates: List[date_type] = Field(
        default_factory=list, description="Dates with missing data"
    )
...
class DataResponse(BaseModel):
    ...
    ohlc_data: Optional[List[OHLCCandle]] = Field(
        None, description="OHLC candles if requested"
    )
    fundamentals: Optional[Fundamentals] = Field(
        None, description="Fundamentals if requested"
    )
    ...
    @classmethod
    def validate_ohlc_sorted(
        cls, v: Optional[List[OHLCCandle]]
    ) -> Optional[List[OHLCCandle]]:
...
class DataError(BaseModel):
    """Structured error from DataAgent. NEVER throw exceptions."""
    ...
    missing_fields: Optional[List[str]] = Field(
        None, description="Fields that were unavailable"
    )
    original_request: Optional[DataRequest] = Field(
        None, description="Request that caused error"
    )
...
def validate_data_response(response: DataResponse) -> List[str]:
    ...
    if (
        RequestedField.FUNDAMENTALS in request.requested_fields
        and response.fundamentals is None
    ):
    ...
    if response.ohlc_data:
        ...
        if len(dates) < expected_days * 0.5:  # Less than 50% coverage
            warnings.append(
                f"Only {len(dates)} days of data out of {expected_days} expected"
            )
