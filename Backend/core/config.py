...
class Settings(BaseSettings):
    ...
    # Local
    LOCAL_MODEL_ENDPOINT: Optional[str] = Field(None, description="Local model endpoint URL")
    
    # Safety & Limits
    MAX_TOKENS: int = Field(2048, ge=1, le=32000, description="Max tokens per response")
    MAX_COST_PER_REQUEST: Optional[float] = Field(None, description="Max cost in USD per request")
