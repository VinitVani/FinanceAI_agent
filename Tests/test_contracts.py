...
from Backend.data.contracts import (
    DataRequest, AssetType, RequestedField, 
    OHLCCandle, DataResponse, DataMetadata, 
    DataError, ErrorCode, 
    validate_data_response, validate_data_error
)

class TestContracts:
    def test_validate_ticker(self):
        with pytest.raises(ValueError, match="Ticker cannot be empty"):
            DataRequest(ticker="", asset_type=AssetType.STOCK, start_date=date.today(), end_date=date.today())
        
        with pytest.raises(ValueError, match="Ticker must be 20 characters or less"):
            DataRequest(ticker="A" * 21, asset_type=AssetType.STOCK, start_date=date.today(), end_date=date.today())
            
        with pytest.raises(ValueError, match="Ticker contains invalid characters"):
            DataRequest(ticker="BTC$USD", asset_type=AssetType.STOCK, start_date=date.today(), end_date=date.today())
    ...
    def test_validate_date_range(self):
        ...
        with pytest.raises(ValueError, match="end_date must be after start_date"):
            DataRequest(ticker="AAPL", asset_type=AssetType.STOCK, start_date=d2, end_date=d1)
            
        # Future end date
        future = date.today() + timedelta(days=1)
        with pytest.raises(ValueError, match="end_date cannot be in the future"):
            DataRequest(ticker="AAPL", asset_type=AssetType.STOCK, start_date=date.today(), end_date=future)
    ...
    def test_validate_data_response_warnings(self):
        req = DataRequest(
            ticker="AAPL", asset_type=AssetType.STOCK, 
        ...
        resp = DataResponse(request=req, metadata=meta, ohlc_data=None, fundamentals=None)
        ...
        resp2 = DataResponse(
            ...
            ohlc_data=[OHLCCandle(date=date(2023, 1, 1), open=1, high=1, low=1, close=1, volume=1)],
            fundamentals=None
        )
    ...
    def test_validate_data_error(self):
        # Good error
        err = DataError(error_code=ErrorCode.API_FAILURE, message="Long enough failure message", retryable=True)
        assert validate_data_error(err) is True
        
        # Short message
        err_short = DataError(error_code=ErrorCode.API_FAILURE, message="Short", retryable=True)
        assert validate_data_error(err_short) is False
        
        # Retryable code but retryable=False
        err_retry = DataError(error_code=ErrorCode.API_FAILURE, message="Valid failure message here", retryable=False)
